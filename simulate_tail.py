     ...
     while True:
        ...
        for filename in os.listdir(path_source_access): 
            ...
            if(strftime("%M", localtime()) < "30" ):
                g=open("access" + strftime("%Y-%m-%d_%H-", localtime()) + "00.txt",'a')
            else : 
                g=open("access" + strftime("%Y-%m-%d_%H-", localtime()) + "30.txt",'a')
            lines = f.readlines()
...
def process2():
    ...
    while True:
        ...
        for filename in os.listdir(path_source_event):
            ...
            #g=open("30.txt",'a')
            if(strftime("%M", localtime()) < "30" ):
                g=open("event" + strftime("%Y-%m-%d_%H-", localtime()) + "00.txt",'a')
            else : 
                g=open("event" + strftime("%Y-%m-%d_%H-", localtime()) + "30.txt",'a')
            lines = f.readlines()
            start_writing_time = 0
            sleep_time = 1
            for line in lines:
